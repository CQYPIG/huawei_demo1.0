<template>
    <div class="col">
        <div class="mylist">
            <a class="listItem active" ref="defalut" href="#" @click.prevent="active" @click="all">
                全部有效订单
            </a>
            <a class="listItem" href="#" @click.prevent="active" @click="ywc">
                已完成
            </a>
            <a class="listItem" href="#" @click.prevent="active" @click="yqx">
                已取消
            </a>
        </div>
        <div>
            ...
            <div v-for="order in orders" :key="order.id">
                <div class="list-top">
                    <div class="list-top-left my">
                        <input type="checkbox" name="payCheck" paymentstatus="2" clearorder="false" value="28781324342">
                ...
                <div class="list">
                    ...
                    <div style="width: 153px;border-right: 0;padding: 10px 0;" class="tbody-td div-td last-td my">
                        ...
                        <!--删除订单 订单已完成 或者订单以关闭 -->
                        <p v-if="(order.yx == true && order.dfk == false && order.dsh == false) || order.yx == false">
...
export default {
    ...
    methods: {
        ...
        cancel(order) {
            if (confirm('是否确定取消订单？')) {
                this.$store.myorder.commit('cancel', order.orderId)
        ...
        //订单支付
        pay(order) {
            this.$store.myorder.commit('pay', order.orderId)
            for (let i = 0; i < this.orders.length; i++) {
                //修改指定订单编号的订单的状态
                if (this.orders[i].orderId == order.orderId) {
                    console.log("想要的代码执行了");
        ...
        deleteOrder(order) {
            if (confirm('是否确定删除订单？此操作不可逆！')) {
                ...
                this.$store.myorder.commit('deleteOrder', order.orderId)
